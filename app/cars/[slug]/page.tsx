...
import { FaUsers, FaCog, FaGasPump, FaCheckCircle, FaCalendarAlt, FaArrowLeft ,FaWhatsapp, FaPhone} from 'react-icons/fa';
...
export default async function CarDetailPage(context: any) {
  const params = await context.params;
  const car = await getCar(params.slug);
        ...
        <section className="section-padding">
          <div className="container-custom">
            <div className="grid lg:grid-cols-2 gap-12">
                ...
                {car.images && car.images.length > 0 && (
                  <div className="grid grid-cols-4 gap-4">
                    {car.images.slice(0, 4).map((image: string, index: number) => (
                      <div key={index} className="relative h-24 rounded-lg overflow-hidden cursor-pointer hover:opacity-75 transition-opacity">
                        <Image
                          src={image}
              ...
              <div>
                <div className="mb-6">
                  <h1 className="text-4xl font-bold text-primary mb-2">{car.name}</h1>
             ...
             <a
              ...
              `Hi, I'm interested in renting the ${car.name} for  day. Is it available?`
                )}`}
                target="_blank"
      ...
      <span>8415038275 </span>
                  ...
                  {car.pricePerWeek && (
                    <div className="mt-2 text-gray-700">
                      Weekly: <span className="font-semibold">{formatCurrency(car.pricePerWeek)}</span>
                    </div>
                  )}
                  {car.pricePerMonth && (
                    <div className="text-gray-700">
                      Monthly: <span className="font-semibold">{formatCurrency(car.pricePerMonth)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="text-2xl font-bold text-primary">{car.seats}</div>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="flex items-center space-x-3 mb-2">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="text-2xl font-bold text-primary">{car.year}</div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    {car.color && (
                      <div>
                        ...
                        <div className="font-semibold text-primary">{car.color}</div>
                      </div>
                    )}
                    {car.mileage && (
                      <div>
                        <div className="text-sm text-gray-600 mb-1">Mileage</div>
                        <div className="font-semibold text-primary">{car.mileage}</div>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="mt-12 pt-12 border-t border-gray-200">
              <h2 className="text-2xl font-bold text-primary mb-4">About This Car</h2>
        ...
        {similarCars.length > 0 && (
          <section className="section-padding bg-gray-50">
            <div className="container-custom">
              <h2 className="text-2xl font-bold text-primary mb-8">Similar Cars You Might Like</h2>
              <div className="grid md:grid-cols-3 gap-8">
                {similarCars.map((similarCar: any) => (
                  ...
                  >
                    ...
                    <div className="p-6">
                      <h3 className="text-lg font-bold text-primary mb-2">{similarCar.name}</h3>
                      <div className="flex items-center justify-between">
                        <span className="text-gray-600 text-sm">{similarCar.type}</span>
                        <span className="text-xl font-bold text-gold">
                          {formatCurrency(similarCar.pricePerDay)}/day
                        </span>
                      </div>
                    </div>
                  </Link>
